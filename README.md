...
* [`aiida_restapi/`](aiida_restapi/): The main source code of the plugin package
  * [`data/`](aiida_restapi/data/): A new `DiffParameters` data class, used as input to the `DiffCalculation` `CalcJob` class
  * [`calculations.py`](aiida_restapi/calculations.py): A new `DiffCalculation` `CalcJob` class
  * [`cli.py`](aiida_restapi/cli.py): Extensions of the `verdi data` command line interface for the `DiffParameters` class
  * [`helpers.py`](aiida_restapi/helpers.py): Helpers for setting up an AiiDA code for `diff` automatically
  * [`parsers.py`](aiida_restapi/parsers.py): A new `Parser` for the `DiffCalculation`
...
```shell
verdi data restapi list
verdi data restapi export <PK>
```
